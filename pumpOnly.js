...
//*********negretive pressure differntial************

const errorNPD = () => {
    var x = d.getElementById("errorNPD");
    // Add the "show" class to DIV
    x.className = "show";
    // After 3 seconds, remove the show class from DIV
    setTimeout(function () { x.className = x.className.replace("show", ""); }, 1000);
...
const looky = () => {
    ...
    if (d.getElementById("money").value === "p") {
        money = "£"
        echangeRate = 1
    }
    if (d.getElementById("money").value === "d") {
        money = "$"
        echangeRate = 1.40 // set to do nothing 
    }
    if (d.getElementById("money").value === "e") {
        money = "€"
    ...
    if (inletFlowDay > 2.5000000001) { // guard to stop error, needs sence check - see ben/tony for correct figure
        ...
        if (chosenPump != 0) {
            ...
            if (chosenPump.length > 0) { // this checks if there is a figure to choose from
                ...
                chosenLine = newPump.filter(a => a[0] === chosenPump[chosenPump.length - 1][0] && a[1] === chosenPump[chosenPump.length - 1][1] && a[2] === pressureDifferential)
